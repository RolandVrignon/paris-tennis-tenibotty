...
Use one prepared request and one cron. Show both account display names/ids, their tariffs, each guest list and the two hours in the user summary. Two hours must be requested explicitly; never add this option to an existing single-hour job merely because a second account was configured. `hours` remains first-hour preferences. The second reservation is the immediately next hour on the court and sport selected by the shared search, including after a primary fallback; it does not require the first hour to succeed. No cross-midnight start at 23h, no independent second-hour club fallback. Paid profiles require an existing compatible ticket book; the bot never purchases one. The two booking accounts must belong to distinct participating players. Do not claim extra quota or confirmed padel quota rules.

A shared read-only search chooses the court and requested hour pair, using the monitoring account when configured. It observes both hours, prefers a complete pair within each starting-hour preference, and can also select a court with only the second hour available. After discovery, two separate Chromium browsers log in, recheck account-specific availability and tariff, and book in parallel. Neither confirmation waits for the other. Keep either confirmed reservation, including a second-hour-only success. Do not add an independent club fallback or change the chosen pair after booking starts.
