...
export const prepareBookingJob = async (input, overrides = {}) => {
  ...
  const normalized = normalizeBookingRequest(input, { now })
  const catalog = overrides.catalog || await loadClubCatalog()
  const resolved = resolveLocations(catalog, normalized.locations)
  for (const club of resolved.clubs) validateClubSport(club.name, normalized.sport)
  if (normalized.sport === 'padel') {
    for (const club of resolved.clubs) selectClubCourts(catalog, club.name, { sport: normalized.sport, courtNumbers: Array.isArray(resolved.locations) ? [] : resolved.locations[club.name] })
  }
  const request = { ...normalized, locations: resolved.locations }
  ...
  try {
    ...
    return {
      requestId,
      cronName: `${request.sport === 'padel' ? 'Padel' : 'Tennis'} ${request.date} ${request.hours.join('/')}h - ${clubs}`,
      schedule: scheduleAt,
      bookingOpensAt,
...
export const cancelBookingJob = (requestId, overrides = {}) => {
  ...
  try {
    ...
    rmSync(getPaths(requestId, options).scriptFile, { force: true })
    return updated
  } finally { release() }
}
...
export const editBookingJob = async (requestId, input, overrides = {}) => {
  const options = bookingJobOptions(overrides)
  const normalized = normalizeBookingRequest(input, { now: overrides.now || dayjs() })
  const catalog = overrides.catalog || await loadClubCatalog()
  const resolved = resolveLocations(catalog, normalized.locations)
  for (const club of resolved.clubs) validateClubSport(club.name, normalized.sport)
  if (normalized.sport === 'padel') {
  ...
  try {
    ...
    const updated = { ...record, request: { ...normalized, locations: resolved.locations }, clubs: resolved.clubs, updatedAt: new Date().toISOString() }
