...
import { mergeConfig, resolveBookingProfile, assertBookingCredentials } from './config.js'
...
export const normalizeBookingRequest = (input, {
  ...
  const accountId = input.bookingAccount ?? 'main'
  if (typeof accountId !== 'string' || !/^[a-z][a-z0-9_-]*$/.test(accountId)) throw new Error('Invalid bookingAccount identifier')
  const profile = fixedConfig ? resolveBookingProfile(fixedConfig, accountId) : undefined
  ...
  if (input.consecutive !== undefined) {
    ...
    if (typeof next.bookingAccount !== 'string' || !/^[a-z][a-z0-9_-]*$/.test(next.bookingAccount) || next.bookingAccount === accountId) throw new Error('consecutive requires a different bookingAccount')
    const nextProfile = fixedConfig ? resolveBookingProfile(fixedConfig, next.bookingAccount) : undefined
    consecutive = { bookingAccount: next.bookingAccount, players: normalizePlayers(next.players === undefined ? nextProfile?.defaultPlayers : next.players) }
  ...
  return {
    ...primary,
    ...(input.bookingAccount !== undefined ? { bookingAccount: accountId } : {}),
...
export const buildBookingConfig = (fixedConfig, request) => {
  ...
  for (const id of [normalized.bookingAccount || 'main', ...(normalized.consecutive ? [normalized.consecutive.bookingAccount] : [])]) assertBookingCredentials(resolveBookingProfile(fixedConfig, id))
