...
export const runTransfer = async (record, session, persist, { now = Date.now, pause = sleep, windowMs = 30000 } = {}) => {
  const change = status => {
  ...
  try {
    ...
    while (now() < deadline) {
      ...
      found = false
      await pause(Math.max(0, Math.min(deadline - now(), 2000 - (now() - started))))
    }
